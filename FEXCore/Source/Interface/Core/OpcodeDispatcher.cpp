  ...
  if (ToSeg) {
    ...
    case FEXCore::X86State::REG_R13: // GS
      if (!Is64BitMode) {
        ...
        // Record the selector but leave the GS base alone: in 64-bit mode the base comes from
        // the GS_BASE MSR and backs the TEB. Failing the decode instead strands loaders that
        // write the selector and read it back.
        _StoreContextGPR(OpSize::i16Bit, Src, offsetof(FEXCore::Core::CPUState, gs_idx));
      }
      break;
